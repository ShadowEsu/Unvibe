            ...
            >
              <div>
                ...
                <div className="mt-4 flex items-center gap-3">
                  ...
                  {f.badge && (
                    <span className="rounded-pill border border-orange/40 bg-orange/10 px-2 py-0.5 text-[0.66rem] font-medium uppercase tracking-wide text-orange">
                      {f.badge}
                    </span>
                  )}
                </div>
                ...
                <ul className="mt-4 flex flex-wrap gap-2">
                  {f.points.map((p) => (
                    <li
                      key={p}
                      className="rounded-pill border border-line bg-surface px-3 py-1 text-fluid-sm text-fg-muted"
                    >
                      {p}
                    </li>
                  ))}
...
function FeatureVisual({ index, feature }: { index: number; feature: Feature }) {
  return (
    <motion.div
      ...
      className="relative aspect-[4/3] overflow-hidden rounded-card border border-line bg-surface p-6"
    >
      ...
      <div className="relative flex h-full flex-col justify-center gap-3">
        ...
        <div className="mt-2 flex gap-1.5">
          {feature.points.slice(0, 2).map((p) => (
            <span
              key={p}
              className="truncate rounded-pill bg-primary-soft px-2.5 py-1 font-mono text-[0.62rem] text-primary"
            >
              {p}
            </span>
          ))}
        </div>
        <span className="absolute right-0 top-0 font-mono text-[0.66rem] text-fg-faint">
          0{index + 1}
        </span>
      </div>
    </motion.div>
