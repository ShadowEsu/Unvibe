/**
 * Free OpenRouter chat models shown in the app picker.
 * Keep in sync with web/src/ai/openrouterModels.ts.
 */
...
export const DEFAULT_CLOUD_MODEL = 'liquid/lfm-2.5-2.6b:free';

export const CLOUD_CHAT_MODELS: CloudChatModel[] = [
  {
...
const ALLOWED = new Set(CLOUD_CHAT_MODELS.map((m) => m.id));

export function normalizeCloudModel(raw?: string | null): string {
  const id = (raw ?? '').trim();
  if (id === 'auto' || id === '') return DEFAULT_CLOUD_MODEL;
  if (ALLOWED.has(id)) return id;
  return DEFAULT_CLOUD_MODEL;
}

export function listCloudChatModels(): CloudChatModel[] {
  return [...CLOUD_CHAT_MODELS].sort((a, b) => a.costRank - b.costRank);
}
