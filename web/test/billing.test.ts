...
test('entitlements match the published Free, Pro, and Teams totals', () => {
  assert.equal(planLimit('free', 'ai_explanation'), 50);
  assert.equal(planLimit('pro', 'ai_explanation'), 100);
  assert.equal(planLimit('pro', 'indexed_project'), 10);
  assert.equal(planLimit('teams', 'project_question', 2), 1_000);
...
test('Teams normalizes abusive seat quantities while Pro stays one-person', () => {
